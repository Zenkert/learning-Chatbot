...
class ActionTellSubjects(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
            ...
            try:
                subject = next(tracker.get_latest_entity_values(entity_type="subject"))
            except:
                subject = None

            if subject is not None:
                dispatcher.utter_message(text=f"I will query my database about {subject}")
                print("Subject: ", subject)

            else:
                dispatcher.utter_message(text=f"These are the some of the subjects available: {subs}")
...
class ActionGiveSuggestion(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
            ...
            subs = main.get_subjects(collection_name='subjects')
            dispatcher.utter_message(text=f"These are some of the subjects I'd suggest: {subs}")
